...
CapooButton::CapooButton()
{
    ...
    button.setScale(WINDOW_WIDTH / 2240, WINDOW_HEIGHT / 1400);
    button.setScale(BUTTON_INIT_SCALE, BUTTON_INIT_SCALE);
}
...
void CapooView::hideCapoo(float delay)
{
    ...
    Components[POO]->addAnimation(0, hideanime, true, delay);
    Components[FOOD]->addAnimation(0, hideanime, true, delay);
...
void CapooView::update(float deltaTime) 
{
    ...
    if (status == EnumCapooStatus::hungry) {
        ...
        if (capooentity.isLeftClick() || elapseTime > 10.f) {
    ...
    bounds = Components[BODY]->getBoundingBox();
